...
(* Parse: wrap parser/lexer; return None on syntax errors *)
let parse (s : string) : expr option =
  let lb = Lexing.from_string s in
  try Some (Parser.prog Lexer.read lb)
  with
  | Parser.Error -> None
  | _ -> None

(* Map a runtime value back into an expr (for substitution semantics) *)
...
(* Substitute value v for free occurrences of variable x in expression e. *)
let rec subst (v : value) (x : string) (e : expr) : expr =
  ...
  | Let (y, e1, e2) ->
      if y = x then (* y shadows x in e2 *)
        Let (y, subst v x e1, e2)
      else
        Let (y, subst v x e1, subst v x e2)
  | Fun (y, body) ->
      if y = x then (* y shadows x in body *)
        Fun (y, body)
      else
        Fun (y, subst v x body)

(* Helpers for bop evaluation *)
      ...
      | Add | Sub | Mul | Div | Mod | Lt | Lte | Gt | Gte | Eq | Neq -> (
          ...
          | Ok v1 -> (
              ...
              | Ok v2 -> (
                  match op with
                  | Add -> (
                      match expect_num v1, expect_num v2 with
