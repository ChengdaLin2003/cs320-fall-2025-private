...
let rec subst (v : value) (x : string) (e : expr) : expr =
  ...
  | Let (y, e1, e2) ->
      if y = x then Let (y, subst v x e1, e2)    (* y 重新绑定，别进 e2 *)
      else Let (y, subst v x e1, subst v x e2)
  | Fun (y, body) ->
      if y = x then Fun (y, body)                (* y 遮蔽 x *)
...
let rec eval (e : expr) : (value, error) result =
  ...
  | App (e1, e2) -> (
      match eval e1 with
      | Ok (VFun (x, body)) -> (match eval e2 with
          ...
          (match eval e1 with
           ...
           | Ok v1 ->
             ...
             | Ok v2 ->
               ...
               | And | Or -> assert false  (* impossible here; handled above *)))
